  ...
  useEffect(() => {
    const fetchAppointment = async () => {
      try {
        ...
        try {
          const messagesResponse = await axios.get(`${API}/chat/messages/${appointmentId}`);
        ...
        {isChatOnly ? (
          ...
          <Card className="rounded-2xl border-none shadow-lg bg-white max-w-4xl mx-auto h-[calc(100vh-180px)]" data-testid="chat-only-area">
            <CardContent className="p-6 h-full flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto mb-4 space-y-3 border rounded-xl p-4 bg-slate-50" data-testid="messages-container">
                {messages.length === 0 ? (
